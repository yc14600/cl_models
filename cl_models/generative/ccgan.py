...
from utils.train_util import shuffle_data,concat_cond_data,one_hot_encoder,get_next_batch
...
class Continual_CGAN(object):
    def __init__(self,x_ph,g_net_shape,d_net_shape,batch_size,conv=False,sess=None,ac_fn=tf.nn.relu,\
                    ...
                    gamma0=1.,alpha=0.01,mem_replay=0.,*args,**kargs):
    ...
    def gen_samples(self,conds,x_shape,c_dim=10):
        samples = []
        labels = []
        print('conds',conds)
        ...
        iters = int(np.ceil(c_sample_size/self.batch_size))
        print('c_sample_size',c_sample_size,'iters',iters)
        for i in conds:
            print('cond',i)
            e = np.random.normal(size=(c_sample_size,self.model.g_W[0].shape[0].value-c_dim)).astype(np.float32)
            c = one_hot_encoder(np.ones(c_sample_size)*i,c_dim)
            e = concat_cond_data(e,c,one_hot=False,dim=c_dim)
            x  = np.zeros(x_shape).astype(np.float32)
            ii = 0            
            for _ in range(iters):
                start = ii
                e_batch,_,ii = get_next_batch(e,self.batch_size,ii,None,repeat=True)
                end = ii if ii < c_sample_size and ii > start else c_sample_size
    ...
    def merge_train_data(self,new_X,new_cond,conds,c_dim=10,save_samples=True,path='./'):
        ...
        if len(conds) > 0:
            ...
            px,py = self.gen_samples(conds,x_shape=new_X.shape,c_dim=c_dim)
            if save_samples:
